...
def finetune_model(model_name, tokenizer, SPARSITY, device=DEVICE, EPOCH_COUNT=10,max_step=50):
    #encode tokens
...
def finetune_model_inplace(model, tokenizer, SPARSITY, device=DEVICE, EPOCH_COUNT=10,max_step=50):
    #encode tokens
    def encode_tok(examples):
        return tokenizer(examples['text'], truncation=True, padding='max_length')
